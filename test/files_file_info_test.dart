import 'package:test/test.dart';
import 'package:onepanel_client/onepanel_client.dart';

// tests for FilesFileInfo
void main() {
  final instance = FilesFileInfoBuilder();
  // TODO add properties to the builder and call build()

  group(FilesFileInfo, () {
    // String content
    test('to test the property `content`', () async {
      // TODO
    });

    // String extension_
    test('to test the property `extension_`', () async {
      // TODO
    });

    // int favoriteID
    test('to test the property `favoriteID`', () async {
      // TODO
    });

    // String gid
    test('to test the property `gid`', () async {
      // TODO
    });

    // String group
    test('to test the property `group`', () async {
      // TODO
    });

    // bool isDetail
    test('to test the property `isDetail`', () async {
      // TODO
    });

    // bool isDir
    test('to test the property `isDir`', () async {
      // TODO
    });

    // bool isHidden
    test('to test the property `isHidden`', () async {
      // TODO
    });

    // bool isSymlink
    test('to test the property `isSymlink`', () async {
      // TODO
    });

    // int itemTotal
    test('to test the property `itemTotal`', () async {
      // TODO
    });

    // BuiltList<FilesFileInfo> items
    test('to test the property `items`', () async {
      // TODO
    });

    // String linkPath
    test('to test the property `linkPath`', () async {
      // TODO
    });

    // String mimeType
    test('to test the property `mimeType`', () async {
      // TODO
    });

    // String modTime
    test('to test the property `modTime`', () async {
      // TODO
    });

    // String mode
    test('to test the property `mode`', () async {
      // TODO
    });

    // String name
    test('to test the property `name`', () async {
      // TODO
    });

    // String path
    test('to test the property `path`', () async {
      // TODO
    });

    // int size
    test('to test the property `size`', () async {
      // TODO
    });

    // String type
    test('to test the property `type`', () async {
      // TODO
    });

    // String uid
    test('to test the property `uid`', () async {
      // TODO
    });

    // String updateTime
    test('to test the property `updateTime`', () async {
      // TODO
    });

    // String user
    test('to test the property `user`', () async {
      // TODO
    });

  });
}
